    ...
    void Game::init(const char* title, int xpos, int ypos, int width, int height, bool fullscreen) {
        ...
        // âm thanh
        if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 16000) < 0) {
        ...
        // Tải file nhạc nền
        backgroundMusic = Mix_LoadMUS("Glorious Morning1.ogg"); // Cập nhật đường dẫn nếu cần
        if (backgroundMusic == nullptr) {
            ...
            backgroundTexture = IMG_LoadTexture(renderer, "paint/1.png");
            ...
            isRunning = true;
            player = new Player(WINDOW_WIDTH / 2 - 37, WINDOW_HEIGHT - 75, 75, 75);

            createEnemies(Num_of_ennemy);
    ...
    void Game::createEnemies(int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (i % 3 == 0) {
                ...
                // Mặc định nếu không thỏa mãn điều kiện nào khác
...
void Game::checkCollisions() {
    for (auto& bullet : bullets) {
        ...
        // Đạn của enemy bắn trúng player
        ...
        // Đạn của player bắn trúng enemy
        else {
            for (auto& enemy : enemies) {
                if (!enemy->isActive()) continue;

                SDL_Rect bulletRect = bullet->getCollider();
                SDL_Rect enemyRect = enemy->getCollider();
                if (SDL_HasIntersection(&bulletRect, &enemyRect)) {

                    enemy->takeDamage(1);
                    bullet->startExplosion();
//                    bullet->setActive(false); // Chỉ setActive(false) ở đây
//                    break;
                }
            }
        }
